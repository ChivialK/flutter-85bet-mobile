...
class RouterConfigResolver {
  ...
  Future<RouteConfig> resolve(FieldElement field) async {
    ...
    if (type.element is! ClassElement) {
      throw ('\n[${type.getDisplayString(withNullability: false)}] is not a class');
    }
    ...
    if (_routeConfig.isUnknownRoute) {
      ...
      if (params.isEmpty ||
          params.length > 1 ||
          params.first.type.getDisplayString(withNullability: false) !=
              'String') {
    ...
    guardsChecker
      ...
      _routeConfig.guards.add(RouteGuardConfig(
          ...
          import: getImport(guard.element)));
    });
    ...
    _routeConfig.className = type.getDisplayString(withNullability: false);

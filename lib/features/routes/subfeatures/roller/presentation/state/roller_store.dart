...
import '../../data/models/roller_prize_winner_model.dart';
...
abstract class _RollerStore with Store {
  ...
  final StreamController<List<RollerPrizeWinnerModel>> _winnersController =
      StreamController<List<RollerPrizeWinnerModel>>.broadcast();
  ...
  _RollerStore(this._repository) {
    _winnersController.stream.listen((event) {
  ...
  Stream<List<RollerPrizeWinnerModel>> get winnersStream =>
      _winnersController.stream;
  ...
  List<RollerPrizeWinnerModel> winners;
...
//        debugPrint('roller init data result: $result');
  ...
  @action
  Future<void> getWinnersList() async {
    try {
      // Reset the possible previous error message.
      errorMessage = null;
      // Fetch from the repository and wrap the regular Future into an observable.
      // ObservableFuture extends Future - it can be awaited and exceptions will propagate as usual.
      await _repository.getWinners().then((result) {
//        debugPrint('roller init data result: $result');
        result.fold(
          (failure) => setErrorMsg(msg: failure.message, showOnce: true),
          (list) {
            debugPrint('roller prize winners: ${list.length}');
            if (list != null &&
                list is List &&
                list.every((element) => element is RollerPrizeWinnerModel) &&
                winners != list) {
              _winnersController.sink.add(list);
            } else {
              _winnersController.sink.add([]);
          ...
          .then((result) => result.fold(
                ...
                (data) {
                  ...
                  if (data != null && requirement != data) {
                    ...
                    _requirementController.sink
                        .add(RollerRequirementModel(hasData: false));
                    setErrorMsg(msg: localeStr.messageWarnNoHistoryData);
                  }
                },
              ))

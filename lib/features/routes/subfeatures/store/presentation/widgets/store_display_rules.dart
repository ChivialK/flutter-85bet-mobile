...
class _StoreDisplayRulesState extends State<StoreDisplayRules> {
  ...
  void updateContent() {
    ...
    _tableWidthMap = new Map<int, FixedColumnWidth>();
    for (int i = 0; i < _store.rulesModel.platformRules.length; i++) {
      _tableWidthMap.putIfAbsent(i, () => FixedColumnWidth(_tableCellWidth));
    }
    debugPrint('table cells: ${_tableWidthMap.length}');

    _headerRowTexts = List.generate(
      _store.rulesModel.platformRules.length,
      (index) =>
